...
class GestureDrawingApp(DrawingApp):
    def __init__(self, master):
        ...
        self.instruction_text = self.canvas.create_text(
            ...
            text="Say 'START' to start drawing.\nSay 'SQUARE' or 'CIRCLE' to draw a square or circle." , font=("Arial", 15), fill="gray", anchor="n"
        )
    ...
    def handle_command(self, command):
        ...
        if command == "START":
            ...
            if not self.square_drawing_enabled:
                ...
                self.update_instruction("Square drawing mode on. Move your thumb and index finger to adjust the size.\nSay 'SQUARE' to finalize.")

            else:
                self.square_drawing_enabled = False
                self.finalize_square()
                self.update_instruction("Square drawing finalized.\nSay 'SQUARE' to start again.\nSay 'START' to start drawing.")
            ...
            if not self.circle_drawing_enabled:
                ...
                self.update_instruction("Circle drawing mode on. Move your thumb and index finger to adjust the size.\nSay 'CIRCLE' to finalize.")
            else:
                self.circle_drawing_enabled = False
    ...
    def change_brush_type(self, brush_type):
        if brush_type in ["solid", "air", "texture", "calligraphy", "blending", "shining"]:
